...
exit;}?>
        ...
        <main class="flex-fill">
            ...
            include "connect.php";
            $data = $_POST;
            $qry = 'UPDATE produtos SET Produto ="'. $data['nomeprod'] .'", Descricao ="'. $data['descprod'] .'", CodTipo = "'. $data['CodTipo'] . '" ,Preco = "'. $data['precoprod'] . '" ,Estoque = "'. $data['estprod'] . '" WHERE CodigoEAN ='. $data['codprod'];
            $pdo->exec($qry);
            echo 'Produto alterado';
            ?><br>
            <button type="button" onclick="window.location.href='userAdminAreaProd.php'" class="btn btn-lg btn-primary">Voltar</button>
            </div>
        </main>
